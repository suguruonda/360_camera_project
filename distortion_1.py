...
if __name__ == "__main__":
    ...
    out_file = '/data2/suguru/datasets/360camera/camera_pram_optimized.h5'
        ...
        for num, j in enumerate(folders):
            ...
            if i == 'camera8':
                ...
                if num == 0:
                    ...
                    if not(store_list[int(camera[-1]) -1]):
                        r1_list[int(camera[-1]) -1] = np.array(c.rvecs[0], dtype=np.float64)
                        t1_list[int(camera[-1]) -1] = t2
                        cp1_list[int(camera[-1]) -1] = c.imgpoints_undistorted[0]
                        p3d1_list[int(camera[-1]) -1] = c.objpoints2[0]
                        store_list[int(camera[-1]) -1] = True
                    else:
                        r2_list[int(camera[-1]) -1] = np.array(c.rvecs[0], dtype=np.float64)
                        t2_list[int(camera[-1]) -1] = t2
                        cp2_list[int(camera[-1]) -1] = c.imgpoints_undistorted[0]
                        p3d2_list[int(camera[-1]) -1] = c.objpoints2[0]

                else:
                    r3, _ = cv.Rodrigues(np.array(c.rvecs[0], dtype=np.float64))
                    t3 = np.array(c.tvecs[0], dtype=np.float64)
                    r_camera3 =  np.dot(r3, r_dif)
                    t_camera3 = t3 + np.dot(r3, t_dif) 
                    if not(store_list[int(camera[-1]) -1]):
                        r1_list[int(camera[-1]) -1] = np.array(c.rvecs[0], dtype=np.float64)
                        t1_list[int(camera[-1]) -1] = t3
                        cp1_list[int(camera[-1]) -1] = c.imgpoints_undistorted[0]
                        p3d1_list[int(camera[-1]) -1] = c.objpoints2[0]
                        store_list[int(camera[-1]) -1] = True
                    else:
                        r2_list[int(camera[-1]) -1] = np.array(c.rvecs[0], dtype=np.float64)
                        t2_list[int(camera[-1]) -1] = t3
                        cp2_list[int(camera[-1]) -1] = c.imgpoints_undistorted[0]
                        p3d2_list[int(camera[-1]) -1] = c.objpoints2[0]
