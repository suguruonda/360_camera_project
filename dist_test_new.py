...
import bundle_adjustment_8poses_2
...
class CB_calbration:
    ...
    def cal_extrinsic_parameters(self):
        ...
        rvecs, tvecs = cv.solvePnPRefineVVS(np.array(self.objpoints, dtype=np.float64), np.reshape(np.array(self.imgpoints, dtype=np.float64), (1,self.c*self.r,2)), self.mtx, self.dist, rvecs, tvecs)
        #rvecs, tvecs = cv.solvePnPRefineLM(np.array(self.objpoints, dtype=np.float64), np.reshape(np.array(self.imgpoints, dtype=np.float64), (1,self.c*self.r,2)), self.mtx, self.dist, rvecs, tvecs)
...
if __name__ == "__main__":
    ...
    if flag_opt:
        out_file = '/data2/suguru/datasets/360camera/camera_pram_2_no180_2_opt_test_vvs.h5'
        #out_file = '/data2/suguru/datasets/360camera/camera_pram_2_no180_2_opt_test_vlm.h5'
    else:
        out_file = '/data2/suguru/datasets/360camera/camera_pram_2_no180_2.h5'
    ...
    if flag_distortion_cal:
        ...
        f_r = h5py.File(out_file_t, 'r')
        for i in range(1,9):
            grp = 'camera'+str(i)
            f.create_group(grp)
            f[grp].create_dataset('mtx', data=f_r[grp]['mtx'][:])
            f[grp].create_dataset('dist', data=f_r[grp]['dist'][:])
    ...
    if flag_opt:
        ...
        #Rs, R_s, Ts, T_s = bundle_adjustment_8poses_2.optimize2(stores_dict)
