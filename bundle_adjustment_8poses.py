...
class pose8(torch.nn.Module):
    ...
    def cal_initial_diffs(self):
        ...
        r_dif_6 = torch.t(r2s[6 + k]) @ R6
        t_dif_6 = torch.t(r2s[6 + k]) @ (T6 - t2s[6 + k])
        #take average with 6 and 5 using quaterion later
        r_dif_5 = torch.t(r2s[5 + k]) @ R5
        t_dif_5 = torch.t(r2s[5 + k]) @ (T5 - t2s[5 + k])
        
        R4 = r1s[4 + k] @ r_dif_5
        T4 = t1s[4 + k] + r1s[4 + k] @ t_dif_5
        R3 = r1s[3 + k] @ r_dif_6
        T3 = t1s[3 + k] + r1s[3 + k] @ t_dif_6
